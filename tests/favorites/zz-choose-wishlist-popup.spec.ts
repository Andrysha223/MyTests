...
test('Выбор списка бажань при добавлении товара, если списков несколько (web1-bi.ua)', async ({
  ...
  // На мобільній верстці сторінка "Список бажань" рендерить списки в
  // принципово іншій DOM-структурі (наприклад, ".lGrBr.showWLJ" замість
  // ".WLwrapper"/".WLtitle" на десктопі), і видимої кнопки видалення ЦІЛОГО
  // списку (на відміну від видалення товару всередині нього) там не знайшлося —
  // судячи з усього, управління кількома списками влаштоване інакше (можливо,
  // через свайп/інший жест), а не через просте доопрацювання селекторів.
  // Сам функціонал (вибір списку в попапі) вже покритий на десктопі.
  test.skip(
