...
//
// Запускает файлы в алфавитном порядке, а этот тест создаёт на
// общем аккаунте лишние списки бажань (удаляются в afterEach). Если прогон
// прервётся ДО afterEach (например, кто-то остановит процесс руками), эти
// списки останутся и сломают все остальные тесты избранного, которые
// предполагают ровно один список (см. BUGS.md). Запуская этот файл строго
// последним среди tests/favorites, мы гарантируем, что даже такое прерывание
// не испортит состояние для тестов, которые уже успели пройти в этом прогоне.
...
test('Выбор списка бажань при добавлении товара, если списков несколько (web1-bi.ua)', async ({
  ...
  // На мобильной верстке страница "Список бажань" рендерит списки в
  // принципиально другой DOM-структуре (например, ".lGrBr.showWLJ" вместо
  // ".WLwrapper"/".WLtitle" на десктопе), и видимой кнопки удаления ЦЕЛОГО
  // списка (в отличие от удаления товара внутри него) там не нашлось —
  // судя по всему, управление несколькими списками устроено иначе (возможно,
  // через свайп/другой жест), а не через простую доработку селекторов.
  // Сам функционал (выбор списка в попапе) уже покрыт на десктопе.
  test.skip(
