...
// Цена товара (1 902 ₴) сама по себе выше порога бесплатной доставки Укрпошти
// (freeShippingMinPrice: 1000 ₴ в ответе API, ниже, чем у Нової Пошти) —
// поэтому одной штуки достаточно, докупать количество не нужно.
...
// Проверяет бесплатную доставку у відділення Укрпошта при сумі замовлення від
// 1000 ₴ (freeShippingMinPrice у відповіді GET /api/v1/basket/delivery, у
// Укрпошти цей поріг нижче, ніж у Нової Пошти — 1000 ₴ проти 1700 ₴): логин ->
// очистка корзины -> добавление дорогого товара (LEGO Marvel Шолом Залізної
// Людини, 1902 ₴, вище порогу) -> контактні дані предзаповнені аккаунтом ->
