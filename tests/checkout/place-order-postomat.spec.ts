...
// Перевіряє оформлення замовлення з доставкою у поштомат «Нова Пошта»: логін ->
// очищення кошика -> додавання LEGO Speed Champions McLaren Senna (75892) ->
// контактні дані вже заповнені акаунтом -> вибір способу доставки
// "У поштоматі «Нова Пошта»", перший доступний поштомат -> перевірка вартості
// доставки в чекауті (як в place-order-nova-poshta) -> на кроці оплати для
// цього способу доставки доступна ЛИШЕ оплата карткою на сайті (LiqPay), без
// варіанту "При отриманні" -> оформлюємо замовлення і СКАСОВУЄМО оплату на стороні
// LiqPay (кнопка "Скасувати оплату"), реальну оплату не проводимо -> LiqPay
// повертає на /thankyou/ зі статусом "Очікування платежу" -> перевіряємо
// підтвердження замовлення.
test('Оформление заказа с доставкою у поштомат «Нова Пошта» без оплати (web1-bi.ua)', async ({
  page,
}) => {
  test.skip(
  ...
  await test.step('Выбрать доставку в поштомат Нової Пошти', async () => {
    const delivery = await checkoutPage.selectPostomatDelivery(CITY);
  ...
  await test.step('Оформить заказ и отменить оплату картой на LiqPay', async () => {
    // У доставки в поштомат немає способу "При отриманні" — лише оплата
    // карткою на сайті (LiqPay), тому реально не платимо, а скасовуємо оплату.
  ...
  await test.step('Проверить подтверждение заказа', async () => {
    ...
    // Сайт показує один і той самий текст способу доставки для відділення і
    // поштомату «Нова Пошта» (не розрізняє їх в підтвердженні), тому
    // перевіряємо тим самим локатором, що і в place-order-nova-poshta.
    await expect(
      thankYouPage.orderDeliveryMethodNovaPoshta,
      'В подтверждении заказа должен быть указан способ доставки «Нова Пошта»',
    ).toBeVisible();
    // Спосіб оплати в підтвердженні повинен бути саме "Карткою на сайті",
    // а не "При отриманні" (якого для поштомата взагалі немає у виборі).
    await expect(
      thankYouPage.orderPaymentMethodCard,
      'В подтверждении заказа должен быть указан способ оплаты «Карткою на сайті»',
    ).toBeVisible();
    // Підтверджуємо, що оплата реально НЕ пройшла (ми її скасували на LiqPay),
    // а не була випадково проведена тестовою карткою.
    await expect(
      thankYouPage.orderPaymentStatusPending,
      'Статус оплати должен быть «Очікування платежу», т.к. оплата картой была отменена на LiqPay, а не проведена',
    ).toBeVisible();
