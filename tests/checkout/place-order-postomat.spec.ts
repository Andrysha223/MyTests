...
// Проверяет оформление заказа с доставкой у поштомат «Нова Пошта»: логин ->
// очистка корзины -> добавление LEGO Speed Champions McLaren Senna (75892) ->
// контактні дані уже предзаполнены аккаунтом -> вибір способу доставки
// "У поштоматі «Нова Пошта»", первый доступный поштомат -> проверка стоимости
// доставки в чекауте (как в place-order-nova-poshta) -> на шаге оплаты для
// цього способу доставки доступна ЛИШЕ оплата карткою на сайті (LiqPay), без
// варіанту "При отриманні" -> оформляем заказ и ОТМЕНЯЕМ оплату на стороне
// LiqPay (кнопка "Скасувати оплату"), реальную оплату не проводим -> LiqPay
// возвращает на /thankyou/ со статусом "Очікування платежу" -> проверяем
...
test('Оформление заказа с доставкою у поштомат «Нова Пошта» без оплати (web1-bi.ua)', async ({
  ...
  await test.step('Оформить заказ и отменить оплату картой на LiqPay', async () => {
    // У доставки в поштомат нет способа "При отриманні" — только оплата
    // карткою на сайті (LiqPay), поэтому реально не платим, а отменяем оплату.
  ...
  await test.step('Проверить подтверждение заказа', async () => {
    ...
    // Сайт показывает один и тот же текст способа доставки для відділення и
    // поштомату «Нова Пошта» (не различает их в подтверждении), поэтому
    // проверяем тем же локатором, что и в place-order-nova-poshta.
    await expect(
      thankYouPage.orderDeliveryMethodNovaPoshta,
      'В подтверждении заказа должен быть указан способ доставки «Нова Пошта»',
    ).toBeVisible();
    // Способ оплаты в подтверждении должен быть именно "Карткою на сайті",
    // а не "При отриманні" (которого для поштомата вообще нет в выборе).
    await expect(
      thankYouPage.orderPaymentMethodCard,
      'В подтверждении заказа должен быть указан способ оплаты «Карткою на сайті»',
    ).toBeVisible();
    // Подтверждаем, что оплата реально НЕ прошла (мы её отменили на LiqPay),
    // а не была случайно проведена по тестовой карте.
    await expect(
