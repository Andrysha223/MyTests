...
test('Применение промокода на 20% скидки меняет цену товара и сохраняется в заказе (web1-bi.ua)', async ({
  ...
  // На мобильной верстке сайдбар "Ваше замовлення" (а вместе с ним и блок
  // "Додати промокод") на шаге 1 вообще не рендерится — он появляется только
  // на шаге 3 (метод оплати), в отличие от десктопа, где виден с шага 1.
  // Тест по смыслу проверяет применение промокода ДО заполнения контактных
  // данных, что на мобильной верстке физически невозможно — это отличие
