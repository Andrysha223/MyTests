...
// Товар для позитивних кроків "додати в кошик -> почати оформлення" —
// сам продукт не важливий, потрібен будь-який доступний самовивозом/у відділення.
...
// Негативні кейси валідації чекауту (гостьовий флоу — авторизація тут не
// потрібна, поля не заповнені акаунтом, що і потрібно для перевірки
// клієнтської валідації). Кожен тест — окремий сценарій, незалежний від
// інших (спільного стану між ними немає, крім кроків "додати в
// кошик -> почати оформлення", які повторюються в кожному).
...
test('Пустая форма контактных данных не проходит валидацию (web1-bi.ua)', async ({ page }) => {
  ...
  await test.step('Нажать "Далі" на пустой форме и проверить ошибки полей', async () => {
    ...
    // Форма не повинна пропустити на крок 2 — поле міста ще не повинно
    // існувати/бути видимим, оскільки попередній крок не пройдено.
    await expect(
...
test('Невалидный формат email не проходит валидацию (web1-bi.ua)', async ({ page }) => {
  ...
  await test.step('Заполнить корректные ФИО/телефон, но невалидный email', async () => {
    // Email — необов'язкове поле (порожнє не дає помилки), але якщо
    // заповнене — повинно проходити перевірку формату.
...
test('Незаполненное поле "Місто" на шаге 2 не проходит валидацию (web1-bi.ua)', async ({ page }) => {
  ...
  await test.step('Нажать "Далі" с пустым полем "Місто"', async () => {
    ...
    // Способи доставки рендеряться лише після вибору міста — їх відсутність
    // підтверджує, що чекаут реально не просунувся далі.
    await expect(
...
test('Гостевой чекаут без согласия с публичным договором не оформляет заказ (web1-bi.ua)', async ({
  ...
  await test.step('Нажать "Оформити замовлення" без галочки согласия', async () => {
    // Чекбокс згоди з "Публічним договором купівлі-продажу" за замовчуванням
    // не відмічений у гостьового чекауту — навмисно НЕ клікаємо по ньому.
