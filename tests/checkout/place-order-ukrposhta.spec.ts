...
// Перевіряє оформлення замовлення з доставкою у відділення Укрпошта (аналогічно
// place-order-nova-poshta.spec.ts, але інший спосіб доставки): логін ->
// очищення кошика -> додавання LEGO Speed Champions McLaren Senna (75892) ->
// перевірка заповнених контактних даних (як в place-order-pickup) ->
// вибір способу доставки "Укрпошта", перше доступне відділення ->
...
test('Оформление заказа с доставкою у відділення Укрпошти (web1-bi.ua)', async ({ page }) => {
  ...
  await test.step('Проверить подтверждение заказа', async () => {
    ...
    // Спосіб доставки в підтвердженні повинен бути саме "Укрпошта",
    // а не самовивіз/Нова Пошта/дефолт — перевіряємо, що вибір реально зберігся.
    await expect(
