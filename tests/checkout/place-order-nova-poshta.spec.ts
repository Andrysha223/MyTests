...
// Перевіряє оформлення замовлення з доставкою у відділення «Нова Пошта» (на
// відміну від інших чекаут-тестів, де перевіряється самовивіз): логін -> очищення
// кошика -> додавання LEGO Speed Champions McLaren Senna (75892) -> контактні
// дані вже заповнені акаунтом -> вибір способу доставки "Нова Пошта",
// перше доступне відділення -> оплата при отриманні -> підтвердження замовлення.
test('Оформление заказа с доставкою у відділення Нової Пошти (web1-bi.ua)', async ({ page }) => {
  ...
  await test.step('Проверить подтверждение заказа', async () => {
    ...
    // Спосіб доставки в підтвердженні повинен бути саме "Нова Пошта",
    // а не самовивіз/дефолт — перевіряємо, що вибір реально зберігся.
    await expect(
