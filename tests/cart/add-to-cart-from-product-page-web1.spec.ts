...
// Перевіряє додавання товару в кошик зі сторінки самого товару на тестовому
// домені web1-bi.ua: спочатку заходимо в каталог і переходимо на сторінку товару
// кліком по картці (а не прямим переходом за URL), потім переконуємось, що
// кнопка "Купити" видна і назва товару збігається з очікуваною, клікаємо по
// кнопці і перевіряємо, що товар потрапив в кошик (за API і в UI) і лічильник
// в хедері оновився.
